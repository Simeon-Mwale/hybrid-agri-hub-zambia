...
// Mock data for demonstration with more realistic structure
let alerts = [
  { 
    id: "1", 
    userId: "u1", 
...
export async function GET(req: Request) {
  try {
    ...
    if (isActive !== null) {
      ...
      filteredAlerts = filteredAlerts.filter(a => a.isActive === active);
    }
    
    if (cropId) {
      filteredAlerts = filteredAlerts.filter(a => a.cropId === cropId);
    }
    
    if (marketId) {
      filteredAlerts = filteredAlerts.filter(a => a.marketId === marketId);
    }
    ...
    if (search) {
      filteredAlerts = filteredAlerts.filter(a => {
        ...
        return (
          ...
          a.targetPrice.toString().includes(search)
        );
      });
    }
    ...
    const enrichedAlerts = paginatedAlerts.map(enrichAlert);
...
export async function POST(req: Request) {
  try {
    ...
    const requiredFields = ['userId', 'cropId', 'marketId', 'targetPrice'];
    ...
    // Validate targetPrice is a number and positive
    ...
    // Check if user, crop, and market exist
    if (!users.find(u => u.id === body.userId)) {
      return NextResponse.json(
    ...
    if (!crops.find(c => c.id === body.cropId)) {
      return NextResponse.json(
    ...
    if (!markets.find(m => m.id === body.marketId)) {
      return NextResponse.json(
...
// PATCH /api/admin/alerts/:id (for toggling status)
...
// PUT /api/admin/alerts/:id (full update)
